...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!csrf_verify()) {
        ...
        if ($result['ok'] ?? false) {
            site_flash_set('success', 'Your account is ready.');
            redirect(resolve_link($next));
        }
        $pageFlash = ['type' => 'error', 'message' => (string) ($result['message'] ?? 'Unable to create your account.')];
    }
}
    ...
    <div class="premium-auth-form-side">
      <div class="premium-auth-box">
        ...
        <div class="premium-auth-tabs">
          <button class="premium-auth-tab" data-target="#loginView">Login</button>
          <button class="premium-auth-tab active" data-target="#registerView">Create Account</button>
          <div class="premium-auth-tab-indicator" style="transform: translateX(100%);"></div>
        </div>

        <div class="premium-auth-forms-container">
          
          <!-- LOGIN VIEW -->
          <div id="loginView" class="premium-auth-view" style="display: none;">
            ...
            <?php if ($pageFlash !== null && false): // Only show register flash here ?>
          ...
          <!-- REGISTER VIEW -->
          <div id="registerView" class="premium-auth-view active">
            <div class="premium-auth-header">
              <div class="ornament">━━ <i class="fas fa-gem" style="font-size:8px;"></i> ━━</div>
              ...
              <div class="premium-field-row">
                <div class="premium-field">
                  ...
                  <div class="premium-input-wrap">
                    ...
                    <input type="text" name="name" required autocomplete="name" placeholder="Enter full name" value="<?= h((string) ($_POST['name'] ?? '')) ?>">
                  </div>
                </div>
                <div class="premium-field">
                  <label>Phone</label>
                  <div class="premium-input-wrap">
                    ...
                    <input type="text" name="phone" required autocomplete="tel" placeholder="Enter phone" value="<?= h((string) ($_POST['phone'] ?? '')) ?>">
                  </div>
                </div>
              </div>
              ...
              <div class="premium-field-row">
                <div class="premium-field">
                  ...
                  <div class="premium-input-wrap">
                    ...
                    <input type="email" name="email" required autocomplete="email" placeholder="Enter email address" value="<?= h((string) ($_POST['email'] ?? '')) ?>">
                  </div>
                </div>
                <div class="premium-field">
                  <label>City</label>
                  <div class="premium-input-wrap">
                    ...
                    <input type="text" name="city" required placeholder="Enter city" value="<?= h((string) ($_POST['city'] ?? '')) ?>">
                  </div>
                </div>
              </div>
